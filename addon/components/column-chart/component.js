    ...
    changeTickForCurrentInterval() {
        ...
        if (this.isIntervalInRange(xscaleTime, indicatorDate)) {
            ...
            if (currentTick && !currentTick.empty() && currentTick.select('text').text().indexOf('\u25C6') === -1) {
                let tickHtml = this.isIntervalIncluded(xscaleTime.ticks(this.get('xAxis').ticks), indicatorDate) ? '<span class="current-interval">`\u25C6 ${currentTick.text()}`</span>' : '<span class="current-interval">\u25C6</span>';
